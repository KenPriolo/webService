...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => navigate("/branch-login"), 4000);
    } catch (err) {
  ...
  return (
    <div className="flex items-center justify-center h-full">
      <form onSubmit={handleSubmit} className="flex flex-col justify-between h-[400px] space-y-4 w-full max-w-md">
        <div>
          <h2 className="text-3xl font-bold text-center mb-4 text-blue-900">Forgot Password</h2>
          <div className="flex items-center justify-center h-full">
        ...
        <div className="space-y-4">
          <div className="flex justify-between">
            ...
            <button type="submit" className="bg-blue-900 text-white px-4 py-2 rounded">Send Link</button>
          </div>
          <p className="text-center text-blue-900 text-sm">
            Remember your password?{" "}
            <button type="button" onClick={() => navigate("/branch-login")} className="underline">Log in</button>
          </p>
        </div>
      </form>
    </div>
